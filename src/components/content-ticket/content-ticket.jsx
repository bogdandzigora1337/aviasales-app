...
function TicketHeader({ price, carrier }) {
  return (
    <div className={cl['content__ticket-header']}>
      <h1 className={cl['content__ticket-price']}>{price.toLocaleString('ru-RU')} P</h1>
      <img src={`http://pics.avs.io/99/36/${carrier}.png`} alt="#" className={cl['content__ticket-logo']} />
    </div>
  )
}
...
export default function ContentTicket() {
  ...
  const allTickets = useSelector((state) => !!state.checkboxReducer.data.length && state.checkboxReducer.data)

  const numberTickets = useSelector((state) => state.moreTicketsReducer.numberTickets)
  ...
  return (
    <>
      {!!tickets.length &&
        tickets.slice(0, numberTickets).map((ticket) => <Ticket key={uniqId()} ticket={ticket}></Ticket>)}
    </>
  )
}
